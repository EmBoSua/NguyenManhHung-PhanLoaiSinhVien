    ...
    public partial class fStudent : Form
    {
        ...
        private void LoadListStudent()
        {
            if (cbMainClass.Items.Count > 0)
            {
                ...
                DataColumn column = table.Columns.Add("Tuổi", typeof(System.Int32));
                foreach (DataRow r in table.Rows)
                {
                    DateTime birthday = DateTime.Parse(r["Ngày sinh"].ToString());
        ...
        private void btSearch_Click(object sender, EventArgs e)
        {
            ...
            Match match = regex.Match(tbSearch.Text);

            if (match.Success && Int32.Parse(tbSearch.Text) > 18 && Int32.Parse(tbSearch.Text) > 65)
            {
                table.DefaultView.RowFilter = "[Tuổi]" + " = '" + tbSearch.Text + "'";
                dgvListStudent.DataSource = table;
            }
            else
            {
                MessageBox.Show("Đây không phải tuổi");
            }*/
            table.DefaultView.RowFilter = "[Tuổi]" + " = '" + tbSearch.Text + "'";
            dgvListStudent.DataSource = table;
        }
