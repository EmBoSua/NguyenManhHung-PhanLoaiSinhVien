...
namespace PhanLoaiSinhVien4
{
    class CreditClassDAO
    {
        ...
        public DataTable LoadClassNew(int kt) // đang hiện toàn bộ các lớp tín chỉ. muốn chỉ lấy lớp chưa vào học thì bỏ comment "--" ở query
        {
            string query;
            if(kt == 2)// kiểm tra đây là class thêm sinh viên vô lớp tín chỉ (1) hay class nhập điểm (2) là xếp lớp tín chỉ
            {
                if (AccountDTO.DangNhap == 1) // admin
                    query = "select m.MaM, m.TenMon, lhp.MaLHP from LopHocPhan as lhp, Mon as m where lhp.MaM = m.MaM  and DATEDIFF(DAY,NgayKT, GETDATE())-20 < 0";
                else // giang vien
                    query = "select m.MaM, m.TenMon, lhp.MaLHP from LopHocPhan as lhp, Mon as m where lhp.MaM = m.MaM  and DATEDIFF(DAY,NgayKT, GETDATE())-20 < 0 and lhp.MaGV = '"+AccountDTO.Ma+"'";
            }else
                query = "select m.MaM, m.TenMon, lhp.MaLHP from LopHocPhan as lhp, Mon as m where lhp.MaM = m.MaM  and DATEDIFF(DAY,NgayBD, GETDATE()) <= 0";
        ...
        public DataTable LoadStudentRegister(string MaM, string MaLHP)
        {
            string query = "exec LoadSinhVienDangKyMon '" + MaM + "', '"+MaLHP+"'";
            DataTable table = new DataTable();
            try
            {
                table = DataProvider.Instance.ExcuteQuery(query);
