...
namespace PhanLoaiSinhVien4
{
    class MainClassDAO
    {
        ...
        public DataGridView AddTo_Km_data_2(string[] column, DataGridView dgv)
        {
            string query = "exec Insert__benngoai @MaSV , @CSLT , @CSDL , @LTHDT , @LTKW , @MMT , @QTM , @LTWE";

            try
            {
                DataProvider.Instance.ExcuteNoneQuery("delete from benngoai");
                int[] ok = new int[dgv.Rows.Count];
                for (int i = 0; i < dgv.Rows.Count; i++)
                {
                    try
